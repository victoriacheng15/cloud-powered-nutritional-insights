...
def get_secret_with_fallback(secret_name: str, env_var_name: str = None, default: str = None) -> str:
    """
    ...
    try:
        ...
        keyvault_secret_name = secret_name.replace("_", "-")
        print(f"[KEY VAULT] Warning: Could not retrieve '{keyvault_secret_name}' from Key Vault: {str(e)}")
        
        # Fallback to environment variable
        if env_var_name:
            env_value = os.getenv(env_var_name)
            if env_value:
                print(f"[KEY VAULT] Using fallback environment variable: {env_var_name}")
                return env_value
        ...
        raise ValueError(f"Could not retrieve secret '{secret_name}' from Key Vault or environment")
