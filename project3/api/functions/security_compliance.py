...
def get_security_status() -> Dict[str, Any]:
    ...
    try:
        ...
        try:
            print(
                "[SECURITY] Attempting to retrieve AZURE_STORAGE_CONNECTION_STRING from Key Vault (will be converted to AZURE-STORAGE-CONNECTION-STRING)"
            )
            ...
            print(
                f"[SECURITY] Successfully retrieved storage connection from Key Vault"
            )
        except Exception as e:
        ...
        is_storage_valid = (
            ...
            != "DefaultEndpointsProtocol=https;AccountName=<storage-account>;AccountKey=<account-key>;EndpointSuffix=core.windows.net"
        )
        ...
        compliance_status = (
            "Compliant"
            if (kv_accessible and keyvault_url and is_storage_valid)
            else "Non-Compliant"
        )
